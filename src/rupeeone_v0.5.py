...
import time # For connection retry delays
import re  # For input validation
from functools import lru_cache  # For data caching
...
class DatabaseManager:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    messagebox.showerror("Database Error", f"Critical database failure after {max_retries} attempts: {e}")
                    self.conn = None
                    self.cursor = None
                    raise SystemExit("Fatal database error")
    ...
    def add_expense(self, date, category, amount, description):
        """Inserts a new expense record into the database with input validation."""
        if not self.cursor: return False
        
        # Input validation
        try:
            # Validate date format (YYYY-MM-DD)
            if not isinstance(date, str) or not re.match(r'^\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01])$', date):
    ...
    def fetch_expenses(self, start_date=None, end_date=None, category=None, year=None, 
                      month_year=None, keyword=None, limit=None, offset=0):
        """Fetches expense records with optional filters and pagination."""
        if not self.cursor: return []
        
        # Base query with better performance using EXISTS
        query = """
    ...
    def get_budget_status(self, month_year):
        """Get comprehensive budget status for a given month.
        
        Returns:
            dict: Contains budget amount, spent amount, remaining amount, and status
        """
        budget = self.get_monthly_budget(month_year)
        expenses = sum(exp[3] for exp in self.fetch_expenses_by_month_year(month_year))
        remaining = budget - expenses
        
        # Determine budget status
        if budget == 0:
            status = "no_budget"
        elif remaining >= budget * 0.2:  # More than 20% remaining
            status = "within_budget"
        elif remaining > 0:  # Less than 20% but still positive
            status = "near_limit"
        else:
            status = "over_budget"
        ...
        return {
            ...
            "remaining": remaining,
            "status": status,
            "percent_used": (expenses / budget * 100) if budget > 0 else 100
        }
    ...
    def get_categories(self):
        """Fetches all categories from the categories table with caching."""
        if not self.cursor: return []
        try:
    ...
    def invalidate_category_cache(self):
        """Clear the category cache when categories are modified."""
...
class ExpenseTrackerApp:
    ...
    def __init__(self, master):
        self.master = master
        master.title("Xpenz - Personal Finance Tracker") # Updated title
    ...
    def _create_expenses_tab_widgets(self, parent_frame):
        ...
        self.remaining_budget_label = ttk.Label(summary_overall_budget_frame, text="₹0.00", style="Value.TLabel", background=self.frame_bg)
    ...
    def set_budget_gui(self):
        """Sets the monthly budget based on user input."""
        try:
            # Get month and year
            selected_month = self.set_budget_month_combobox.get()
            selected_year = self.set_budget_year_combobox.get()
            month_year = f"{selected_year}-{selected_month}"
            
            # Get and validate budget amount
            budget_amount = self.set_budget_entry.get().strip()
            if not budget_amount:
                messagebox.showwarning("Invalid Input", "Please enter a budget amount.")
                return
                
            budget_amount = float(budget_amount)
            if budget_amount < 0:
                messagebox.showwarning("Invalid Input", "Budget amount cannot be negative.")
                return
            
            # Set the budget in the database
            if self.db.set_monthly_budget(month_year, budget_amount):
                self.update_summary_and_budget_display()
                self.update_dashboard_summary()
                self.set_budget_entry.delete(0, tk.END)  # Clear the entry
                self.update_status_bar(f"Budget for {month_year} set to ₹{budget_amount:,.2f}", self.accent_primary)
    ...
    def generate_pie_chart_filtered(self):
        ...
        # --- Logic to combine small slices into "Other" ---
        total_sum = sum(category_totals.values())
        if total_sum == 0:
            self.display_no_chart_data_message("No expenses to display in the pie chart.")
            return

        # Threshold for combining small slices (e.g., 2% of total)
        min_percentage_for_individual_slice = 2.0 
        
        # Separate categories into large and small
        large_slices = {}
        other_sum = 0.0
        
        for label, value in category_totals.items():
            percentage = (value / total_sum) * 100
            if percentage >= min_percentage_for_individual_slice:
                large_slices[label] = value
            else:
                other_sum += value
        
        # Add "Other" slice if there are small categories
        if other_sum > 0:
        ...
        # Custom autopct to handle small percentages better
        def autopct_format(pct):
            return ('%1.1f%%' % pct) if pct >= min_percentage_for_individual_slice else ''
